...
// Tests that don't care about disk-based OG identity (public/og.*, site.json)
// must not silently inherit this template's own real site.json — isolate
// them in an empty scratch dir instead of defaulting to process.cwd().
function emptyCwd() {
  return mkdtempSync(join(tmpdir(), "grok-pwa-test-"));
}
